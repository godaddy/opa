...
type Manager struct {
	...
	reconfigureListeners         []func(*config.Config, *cache.Config)
}
...
func (m *Manager) Reconfigure(config *config.Config) error {
	...
	for _, listener := range m.reconfigureListeners {
		listener(config, interQueryBuiltinCacheConfig)
	}
...
// RegisterReconfigureListener accepts a func that receives new config generated by a reconfigure of the plugin manager, which can be used to propagate config updates across other sections of the code.
func (m *Manager) RegisterReconfigureListener(listener func(*config.Config, *cache.Config)) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
	m.reconfigureListeners = append(m.reconfigureListeners, listener)
}
